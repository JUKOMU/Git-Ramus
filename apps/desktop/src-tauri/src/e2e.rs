...
fn seed_fixture_files() -> Result<SeededFixtureFiles, AppError> {
    ...
    let result = (|| {
        ...
        create_repository(&primary_repository, true)?;
        create_repository(&secondary_repository, false)?;
        create_repository(&excluded_repository, false)?;
        create_repository(&too_deep_repository, false)?;
...
fn create_repository(repository: &Path, dirty: bool) -> Result<(), AppError> {
    fs::create_dir_all(repository)?;
    run_git(repository, ["-c", "init.defaultBranch=main", "init"])?;
...
fn run_git<const N: usize>(repository: &Path, args: [&str; N]) -> Result<(), AppError> {
    let output = SystemGitRunner::new().run(GitCommand {
